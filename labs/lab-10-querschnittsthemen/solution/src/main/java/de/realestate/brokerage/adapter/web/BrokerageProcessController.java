...
public class BrokerageProcessController {
    ...
    @PostMapping
    public ResponseEntity<BrokerageProcessResponse> create(@RequestBody CreateBrokerageProcessCommand command) {
        BrokerageProcessResponse process = service.create(command);
        return ResponseEntity.status(HttpStatus.CREATED).body(process);
    }

    @GetMapping("/{id}")
    public ResponseEntity<BrokerageProcessResponse> findById(@PathVariable UUID id) {
