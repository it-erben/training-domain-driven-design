...
Den Code aus Lab 04 in eine Clean-Architecture-Paketstruktur überführen.
...
Refaktoriere den Code aus Lab 04 in die folgende Paketstruktur:
...
- `domain/model/`: `Address`, `AskingPrice`, `Commission`, `ProcessStatus`,
...
- Injiziert `BrokerageProcessRepository` (Domain-Port-Interface) per Constructor
  Injection
- Methoden:
    -
    `create(UUID propertyId, Address address, AskingPrice askingPrice, Commission commission)` –
...
```bash
cd ../../solutions/lab-05-clean-architecture-refactoring
mvn compile
```
...
3. Der Application Service kann das Repository-Port per Constructor Injection
   empfangen.
...
Überprüfe mit einem einfachen grep/find, dass keine `org.springframework`
-Imports in `domain/` existieren:
