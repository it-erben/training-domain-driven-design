...
public class BrokerageProcess {
    ...
    private final List<BrokerageEvent> domainEvents;
    ...
    /**
     * Factory method to restore a process from persistence.
     */
    public static BrokerageProcess reconstitute(UUID id, UUID propertyId,
                                                AskingPrice askingPrice, Commission commission, ProcessStatus processStatus, List<Viewing> viewings, List<Offer> offers) {
        var process = new BrokerageProcess(id, propertyId, askingPrice, commission);
        process.status = Objects.requireNonNull(processStatus, "Status must not be null");
